...
pacman::p_load(Rcpp, geofacet, tidyverse, dplyr, ggplot2, data.table, lubridate, plotROC, usmap, glmnet, 
               plotly, haven, magrittr, Stat2Data, skimr, GGally, rvest, gridExtra)
```
...
fit1 <- lm(airquality_cmonoxide$Year~ cmonoxide_concentration, data =  airquality_cmonoxide)# model specification response ~ x1,..
ggplot(airquality_cmonoxide , aes(x = Year, y = cmonoxide_concentration, color = cmonoxide_concentration)) +
  ggtitle(" cmonoxide Concentration by Year") +
  geom_line() +
  geom_point()

#Lead concentration by year
fit1 <- lm(airquality_lead$Year~ lead_concentration, data =  airquality_lead)# model specification response ~ x1,..
