...
#Data Partition
set.seed(381)
train <- sample(nrow(glmnet_data), (nrow(glmnet_data)*0.7), replace = F)
glmnet.train <- glmnet_data[train,]
glmnet.test <- glmnet_data[-train,]

fit <- cforest(med.aqi~., data = glmnet.train, controls = cforest_unbiased(ntree = 500, mtry = 59))
library(randomForest)
set.seed(1034)
glmnet.rf <- randomForest(med.aqi~., data = glmnet.train)
```
